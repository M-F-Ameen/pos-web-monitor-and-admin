...
function getDbPath(): string {
  ...
  if (isDev) {
    return path.join(app.getAppPath(), "tobacco_pos.db");
  }
  return path.join(app.getPath("userData"), "tobacco_pos.db");
}
...
const MIGRATIONS: { version: number; description: string; sql: string }[] = [
  ...
  },
  {
    version: 8,
    description:
      "Add high-volume composite indexes for sales/returns reporting",
    sql: `
