...
function normalizeStatuses(
  statuses: ReturnsListQuery["statuses"],
): Return["status"][] {
...
function buildReturnsWhereClause(query?: ReturnsListQuery): {
  whereSql: string;
  params: unknown[];
} {
...
function normalizeReturnStatus(value: unknown): Return["status"] {
...
export function listReturns(query: ReturnsListQuery = {}): Return[] {
  const db = getDb();
...
export function listReturnsPaged(
  ...
  const rows = db
    .prepare(
      `SELECT * FROM returns${whereSql} ORDER BY created_at DESC LIMIT ? OFFSET ?`,
    )
...
export function getReturnById(id: string): Return | null {
  const db = getDb();
  const row = db.prepare("SELECT * FROM returns WHERE id = ?").get(id) as
...
export function createReturn(data: {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    const returnNumber =
      preferredReturnNumber ?? generateReturnNumberCandidate();
    ...
    try {
      const txn = db.transaction(() => {
        ...
        `,
          ...
          data.reason?.trim() ?? "",
          status,
          data.processedById?.trim() ?? "",
          data.processedByName?.trim() ?? data.processedBy?.trim() ?? "",
        );
  ...
  throw new Error(
    `Failed to create return due to number collisions. ${
      lastCollisionError instanceof Error ? lastCollisionError.message : ""
    }`.trim(),
  );
}

export function createReturnsBatch(input: {
  ...
  const txn = db.transaction(() => {
    ...
    for (const entry of entries) {
      ...
      const normalizedRefundAmount =
        Math.round((entry.refundAmount + Number.EPSILON) * 100) / 100;
      ...
      insertReturn.run(
        ...
        entry.reason?.trim() ?? "",
        status,
        entry.processedById?.trim() ?? "",
        entry.processedByName?.trim() ?? entry.processedBy?.trim() ?? "",
      );

      if (status === "approved" && entry.productId) {
        incrementStock(entry.productId, entry.quantity);
      }
...
export function updateReturnStatus(
  ...
  const txn = db.transaction(() => {
    db.prepare("UPDATE returns SET status = ? WHERE id = ?").run(
      normalizedStatus,
      id,
    );
    ...
    if (
      ...
      incrementStock(existing.productId, existing.quantity);
    } else if (
      normalizedStatus !== "approved" &&
      existing.status === "approved" &&
      existing.productId
...
export function deleteAllReturns(): number {
  const db = getDb();
  const txn = db.transaction(() => {
    ...
    for (const row of approvedReturns) {
      ...
      decrementStock(
        ...
        true, // allow negative to prevent blocking the entire operation
      );
    }
