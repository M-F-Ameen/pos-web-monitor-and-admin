...
exports.runOperationalDataRetentionCleanup = runOperationalDataRetentionCleanup;
...
function getCutoffDateTime() {
    ...
    const row = db
        ...
        .get(RETENTION_OFFSET);
    return row.cutoff;
}
function deleteOldRowsInBatches(tableName, dateColumn, cutoffAt, extraWhereSql, affectedIdColumn) {
    const db = (0, database_1.getDb)();
    ...
    for (let batch = 0; batch < RETENTION_MAX_BATCHES_PER_RUN; batch += 1) {
        ...
        const rows = db
          ...
          LIMIT ?
        `)
            .all(cutoffAt, RETENTION_BATCH_SIZE);
        if (rows.length === 0) {
            break;
        }
        if (affectedIdColumn) {
...
function refreshCustomerFinancials() {
    const db = (0, database_1.getDb)();
...
function refreshCustomerFinancialsByIds(customerIds) {
    const db = (0, database_1.getDb)();
...
function refreshSupplierFinancials() {
    const db = (0, database_1.getDb)();
...
function refreshSupplierFinancialsByIds(supplierIds) {
    const db = (0, database_1.getDb)();
 ...
 */
function runOperationalDataRetentionCleanup() {
    const txn = (0, database_1.getDb)().transaction(() => {
        const cutoffAt = getCutoffDateTime();
        const deletedReturnsResult = deleteOldRowsInBatches("returns", "created_at", cutoffAt);
        const deletedSalesResult = deleteOldRowsInBatches("sales", "created_at", cutoffAt, undefined, "customer_id");
        const deletedTreasuryOperationsResult = deleteOldRowsInBatches("treasury_ops", "created_at", cutoffAt);
        const deletedUserShiftsResult = deleteOldRowsInBatches("user_shifts", "logout_at", cutoffAt, "logout_at IS NOT NULL");
        const deletedSupplierOperationsResult = deleteOldRowsInBatches("supplier_operations", "created_at", cutoffAt, undefined, "supplier_id");
        ...
        const refreshedCustomers = affectedCustomerIds.length > 0
            ? refreshCustomerFinancialsByIds(affectedCustomerIds)
            : 0;
        const refreshedSuppliers = affectedSupplierIds.length > 0
